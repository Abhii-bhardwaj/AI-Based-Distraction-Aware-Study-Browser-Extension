...
const DEFAULT_WEIGHTS = {
  tabSwitchFreq: 0.22,
  idleDuration: 0.15,
  scrollIrregularity: 0.1,
  keystrokeVariance: 0.12,
  domainRevisitFreq: 0.15,
  timeOfDayWeight: 0.05,
  tabSwitchSpeed: 0.1,
  notifOpenSpeed: 0.05,
  mouseVelocity: 0.03,
  scrollJerk: 0.02,
  burstSwitchCount: 0.01,
};
...
chrome.tabs.onActivated.addListener(async (info) => {
  ...
  state.tabSwitchTimestamps = state.tabSwitchTimestamps.filter(
    (t) => t > cutoff,
  );
  ...
  chrome.tabs.get(info.tabId, async (tab) => {
    ...
    state.domainVisits[domain] = state.domainVisits[domain].filter(
      (t) => t > cutoff,
    );
...
chrome.notifications.onClicked.addListener(async (notifId) => {
  ...
  if (createTime) {
    ...
    state.notifOpenSpeeds = state.notifOpenSpeeds.filter(
      (e) => e.time > Date.now() - 30 * 60000,
    );
  ...
  if (state.notifications) {
    const notif = state.notifications.find(
      (n) => notifId.includes("tier" + n.tier) && n.complied === null,
    );
    if (notif) {
      notif.complied = true;
      notif.openedAt = Date.now();
    }
  }
  await saveState(state);
});
...
chrome.idle.onStateChanged.addListener(async (idleState) => {
  ...
  if (idleState === "active") {
    ...
    state.idleEvents = state.idleEvents.filter(
      (e) => e.start > now - 5 * 60000,
    );
  } else {
    if (!state.isCurrentlyIdle && state.lastActiveTimestamp) {
      state.activeTimeMs =
        (state.activeTimeMs || 0) + (now - state.lastActiveTimestamp);
    }
  ...
  if (changes.trackingEnabled) {
    ...
    if (enabled) {
      ...
      (async () => {
        ...
        console.log(
          "[StudyGuard] Session started via storage — tracking enabled",
        );
      })();
    } else {
      // Session ended
      (async () => {
        ...
        console.log(
          "[StudyGuard] Session ended via storage — tracking disabled",
        );
      })();
    }
  }
...
function extractFeatures(state) {
  ...
  if (recentTs.length >= 2) {
    ...
    for (let i = 1; i < recentTs.length; i++)
      intervals.push(recentTs[i] - recentTs[i - 1]);
  ...
  const scrollIrregularity =
    scrolls.length > 2
      ? Math.min(stdDev(scrolls.map((e) => e.speed)) / 100, 1.0)
      : 0;

  // 6. scrollJerk — avg jerk (rate of speed change)
  const avgJerk =
    scrolls.length > 1
      ? Math.min(average(scrolls.map((e) => e.jerk || 0)) / 200, 1.0)
      : 0;
  const scrollJerk = avgJerk;

  // 7. keystrokeVariance
  const keys = state.keystrokeTimings.filter((e) => e.time > win);
  const keystrokeVariance =
    keys.length > 3 ? Math.min(stdDev(keys.map((e) => e.delay)) / 500, 1.0) : 0;

  // 8. mouseVelocity — avg mouse speed (high = frantic)
  const mouse = (state.mouseEvents || []).filter((e) => e.time > win);
  const mouseVelocity =
    mouse.length > 2
      ? Math.min(average(mouse.map((e) => e.velocity)) / 2000, 1.0)
      : 0;
  ...
  const timeOfDayWeight =
    h >= 14 && h <= 16 ? 0.6 : h >= 22 || h <= 2 ? 0.8 : 0.3;
...
async function computeDLS(features) {
  const featureArray = [
    features.tabSwitchFreq,
    features.idleDuration,
    features.scrollIrregularity,
    features.keystrokeVariance,
    features.domainRevisitFreq,
    features.timeOfDayWeight,
  ];
  ...
  if (dls >= TIER3) {
    ...
    if (prev < 3) {
      ...
      chrome.notifications.create(nId, {
        ...
        message:
          "Critical distraction! All distracting sites blocked for 5 min.",
        priority: 2,
      });
      ...
      for (const tab of tabs) {
        ...
        if (isDistracting(extractDomain(tab.url))) {
          chrome.tabs.update(tab.id, {
            url: chrome.runtime.getURL("intervention/block.html"),
          });
        }
      }
      try {
        await chrome.declarativeNetRequest.updateEnabledRulesets({
          enableRulesetIds: ["distraction_rules"],
        });
      } catch (e) {}
    }
  } else if (dls >= TIER2) {
...
async function runScoringCycle() {
  ...
  if (dls >= TIER1) {
    ...
    if ((state.prevDLS || 0) < TIER1) {
      const streakMin = Math.round(
        (Date.now() - state.focusStreakStart) / 60000,
      );
  ...
  if (state.notifications) {
    state.notifications.forEach((notif) => {
      if (notif.dlsAfter === null && Date.now() - notif.time > TWO_MINUTES) {
        notif.dlsAfter = dls;
        if (notif.complied === null)
          notif.complied = notif.dlsAfter < notif.dlsBefore - 0.1;
      }
    });
  }

  const completedNotifs = (state.notifications || []).filter(
    (n) => n.complied !== null,
  );
  const complianceRate =
    completedNotifs.length > 0
      ? Math.round(
          (completedNotifs.filter((n) => n.complied).length /
            completedNotifs.length) *
            100,
        )
      : null;
  ...
  const distractionPct =
    state.focusWindowsTotal > 0
      ? Math.round(
          (state.focusWindowsDistracted / state.focusWindowsTotal) * 100,
        )
      : 0;
  const focusScore = Math.max(
    0,
    Math.round(100 - distractionPct * 0.7 - state.interventionCount * 2),
  );
  const patienceIndex =
    state.focusWindowsTotal > 0
      ? Math.round(
          ((state.focusWindowsTotal - state.focusWindowsDistracted) /
            state.focusWindowsTotal) *
            100,
        )
      : 100;
  ...
  const recentNotifSpeeds = (state.notifOpenSpeeds || []).filter(
    (e) => e.time > Date.now() - 10 * 60000,
  );
  const avgNotifOpenSpeed =
    recentNotifSpeeds.length > 0
      ? Math.round(
          (average(recentNotifSpeeds.map((e) => e.speed)) / 1000) * 10,
        ) / 10
      : null;

  // Compute avg tab switch speed for display
  const recentTs = (state.tabSwitchTimestamps || []).filter(
    (t) => t > Date.now() - 5 * 60000,
  );
  ...
  const liveData = {
    ...
    focusScore,
    patienceIndex,
    distractionPercentage: distractionPct,
  ...
  const { sessionHistory = [] } =
    await chrome.storage.local.get("sessionHistory");
  const entry = {
    sessionId: state.startTime,
...
async function runRecalibration() {
  ...
  const avgFocusScore =
    recent.reduce((a, b) => a + (b.focusScore || 0), 0) / recent.length;
  const complianceSessions = recent.filter((s) => s.complianceRate != null);
  const avgCompliance =
    complianceSessions.length > 0
      ? complianceSessions.reduce((a, b) => a + (b.complianceRate || 0), 0) /
        complianceSessions.length
      : 50;
  const avgInterventions =
    recent.reduce((a, b) => a + (b.interventionCount || 0), 0) / recent.length;

  if (avgFocusScore > 75 && avgInterventions > 4) {
    weights.scrollIrregularity = Math.max(
      weights.scrollIrregularity - 0.02,
      0.05,
    );
    weights.keystrokeVariance = Math.max(
      weights.keystrokeVariance - 0.02,
      0.05,
    );
  }
  if (avgCompliance > 70 && avgFocusScore < 50) {
    weights.domainRevisitFreq = Math.min(
      weights.domainRevisitFreq + 0.03,
      0.35,
    );
  ...
  Object.keys(weights).forEach(
    (k) => (weights[k] = Math.round((weights[k] / total) * 1000) / 1000),
  );
